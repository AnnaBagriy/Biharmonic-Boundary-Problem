...
$$
 $u(x)=u(x_1,x_2) \in C^2(\Omega)\cap C(\overline{\Omega}), \ n=(n_1, n_2)$ - зовнішня нормаль до $\Gamma$, 
 $$Mu=\nu\Delta u+(1-\nu)(u_{x_1 x_1}n_1^2+2u_{x_1 x_2}n_1 n_2+u_{x_2 x_2}n_2^2) \textrm{ - згинальний момент пластини } \Omega,
 $$ 
 $\nu\in (0, 1)$ - коефіцієнт Пуассона і $g(x),q(x),f(x)$ - деякі задані функції. 
 
 Задача \eqref{directProblem}-\eqref{dataEq} полягає у знаходженні невідомої $\Gamma_1$ для заданих граничних умов. Дана обернена задача є нелінійною і некоректною.
...
Для чисельного розв'язування отриманої системи інтегральних рівнянь використуємо метод Нистрьома, який полягає у заміні інтегралу вiдповiдною квадратурною формулою з певними ваговими функцiями \cite{kress}. Виберемо тригонометричні квадратури та рівновіддалений поділ,
...
q_n(s)=\sum_{i=0}^{2n}q_{ni}l_i(s), \quad n\in N, \ n>m, \nonumber
 ...
 де $l_i(s)=\cos is$, коли $i=0,...,n$ i $l_i(t)=\sin(n-i)s$ для $i=n+1,...,2n$.
...
A_{ij}=\frac{1}{8m}\sum_{k=0}^{2m-1}\Big(l_{j}(s_k)N_r(s_i,s_k)\varphi_{1m}(s_k)+l_{j}(s_k)\tilde{N}_r(s_i,s_k)\psi_{1m}(s_k)\Big) \nonumber
 ...
 \begin{equation}
 \begin{multlined}
 b_i=f(t_i)-w(t_i)-\sum_{k=0}^{2m-1}\Big(\frac{1}{2m}H_{21}(t_i,t_k)\varphi_{1n}(t_k)+\frac{1}{2m}\tilde{H}_{21}(t_i,t_k)\psi_{1n}(t_k)+\\\big(R_j(t_i)H_{22}^{(1)}(t_i,t_k)+\frac{1}{2m}H_{22}^{(2)}(t_i,t_k)\big)\varphi_{2n}(t_k)+\big(R_j(t_i)\tilde{H}_{22}^{(1)}(t_i,t_k)+\frac{1}{2m}\tilde{H}_{22}^{(2)}(t_i,t_k)\big)\psi_{2n}(t_k)\Big). \nonumber
 \end{multlined}
 \end{equation} 
